...
const CancelledAppointments = () => {
    ...
    const { data: appointments, isLoading, error } = useQuery({
        queryKey: queryKeys.appointments.cancelled(roleData?.id),
        queryFn: () => fetch(`${API_URL}/appointments?doctor_id=${roleData?.id}&status=CANCELLED`,
            {
                method: 'GET',
                credentials: 'include',
                headers: {
    ...
    const CancelledAppointmentCard = ({ name, time, id,patientId }) => {
        return (
            <Paper sx={{ p: 2 }}>
    ...
    return (
        ...
        <Paper variant="outlined" sx={{ p: 2, flex: 1 }}>
            <Typography variant="h5" fontWeight={"bold"}>Cancelled Appointments</Typography>
            {isLoading ?
                ...
                error ?
                    ...
                    appointments.length === 0 ?
                        <Paper sx={{ p: 3 }}>
                            <Typography>You have no cancelled appointments</Typography>
                        </Paper>
                        :
                        <Stack spacing={2}>
                            {
